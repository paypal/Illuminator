...
class AutomationParserFactory
	...
	def self.getLetterDestination(letterMap, letter)
		return letterMap[letter]? letterMap[letter] : letter
	end
	
	def self.saveMyOption(options, letterMap, letter, newval)
		options[self.getLetterDestination(letterMap, letter)] = newval
	end
	
	def self.makeArgumentBank(options, letters = nil, letterMap = nil)
		if letterMap.nil?
			letterMap = {}
		end
		switches = {}
		
		switches["x"] = OpenStruct.new(:opts => ["-x", "--xcode PATH", "Sets path to default Xcode instalation "], 
			:block => lambda { |path| self.saveMyOption(options, letterMap, "x", path) })
		...
		retval.on_tail("-h", "--help", "Show this help message") {|foo| puts retval; exit}
	  ...
	  # build a parser as specified by the user
		letters.each_char do |c|
			if c == "#"
				...
				retval.on(*(switches[c][:opts])) {|foo| switches[c][:block].call(foo)}
 	    end
		end
...
class AutomationArgumentParser
	def parse args
		...
		options = {}
		parser = AutomationParserFactory.makeArgumentBank(options, "x#", {"x" => "defaultXcode"})
...
#		
#		arguments = ["-x", "--xcode PATH", "Sets path to default Xcode instalation "]
#		
#		
...
#				options = self.readFromPath parserPath
#			end
